...
class interpolant():
    ...
    def condition(self, XY, UV):
        ...
        subsets, self.covering_radii_history = smart_thinner(XY, int(XY.shape[0]*0.1))
        ...
        steps = list(range(len(subsets) - 1, 0, -30))
        ...
        for i in steps:
            ...
            sol = np.array(np.split(lu_solve(lu_factor(array), error), N))[:, :, np.newaxis]

            self.interpolants.append(create_interpolant(Y, sol, support_radii))
            mistakos = error - array @ sol.flatten()
